...
void WavetableOscillator::processSingle(float frequency, float detuneRatio, float* output, unsigned nframes)
{
    ...
    nonstd::span<const float> table = multi.getTableForFrequency(frequency);
...
WavetableMulti Wavetables::createFromAudioData(const float* audioData, unsigned audioSize)
{
    // an even size is required for FFT
    if (audioSize % 2 != 0)
        throw std::runtime_error("The wavetable size must be a multiple of 2.");

    size_t fftSize = audioSize;
    ...
    kiss_fftr(cfg, audioData, reinterpret_cast<kiss_fft_cpx*>(spec.get()));
    kiss_fftr_free(cfg);
    ...
    return WavetableMulti::createForHarmonicProfile(hp, 1.0);
}
